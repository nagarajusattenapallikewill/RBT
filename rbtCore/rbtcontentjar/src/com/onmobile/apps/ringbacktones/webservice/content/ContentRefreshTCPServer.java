...
{
	private static final String CLASSNAME = "ContentRefreshTCPServer";
	...
	public ContentRefreshTCPServer(RBTContentCacheManager rbtContentCacheManager, int port, int eventWaitTime)
	{
		...
		{
			RBTLogger.logException(CLASSNAME, "ContentRefreshTCPServer", e);
		}
	}
	...
	private Selector initSelector() throws IOException
	{
		...
		RBTLogger.logDetail(CLASSNAME, "initSelector", "RBT:: Started ContentRefreshTCPServer on port: " + port);
	...
	{
		RBTLogger.logDetail(CLASSNAME, "stop", "RBT:: Stopping ContentRefreshTCPServer");
		runServer = false;
	...
	public void run()
	{
		while (runServer)
		{
			try 
			{
				...
				if (events > 0)
				{
					RBTLogger.logDetail(CLASSNAME, "run", "RBT:: Got " + events + " events");
			...
			{
				RBTLogger.logException(CLASSNAME, "run", e);
			}
		}
	}
	...
	private void accept(SelectionKey key)
	{
		...
		try
		{
			...
			RBTLogger.logDetail(CLASSNAME, "accept", "RBT:: Accepted a connection from " + ipAddress);
		}
		catch (IOException e)
		{
			RBTLogger.logException(CLASSNAME, "accept", e);
		}
	}
	...
	private void read(SelectionKey key)
	{
		...
		try
		{
			...
			RBTLogger.logDetail(CLASSNAME, "read", "RBT:: Read Count = " + readCount);
			...
			RBTLogger.logDetail(CLASSNAME, "read", "RBT:: Got '" + message + "' message from " + clientIPAddress + ":" + clientPort);
		...
		{
			RBTLogger.logException(CLASSNAME, "read", e);
		}
		finally
		{
			try
			{
				RBTLogger.logDetail(CLASSNAME, "read", "RBT::Closing SocketChannel: " + socketChannel);
				socketChannel.close();
