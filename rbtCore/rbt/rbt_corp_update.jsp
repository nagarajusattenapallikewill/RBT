...
if (validateUser(request, session,  "rbt_corp_tunes.jsp", response)) { %>
		<% 
			String strSub=null;String song=null; String file=null; String act_by=null; String corpSplFeatureStr=null;
			boolean bPrepaid=false;boolean bReact=false; boolean bRemSel=false; boolean bBlackoutSMS=false; boolean corpSplFeatureBln=false;
		%>
		...
		try
		{
			...
			corpSplFeatureStr = mySmartUpload.getRequest().getParameter("corpSplFeature"); 
            if(corpSplFeatureStr == null) 
			...
			if(mySmartUpload.getRequest().getParameter("RMV_SEL") != null)
				bRemSel = true;
			...
			if(mySmartUpload.getFiles().getCount() > 0){
				...
				if(mySmartUpload.getFiles().getFile(0).getSize() > 0)
				{
					...
					file = "Corporate-Selection-" + System.currentTimeMillis() + ".txt";
			...
			if(strSub != null){
				...
				System.out.println("Corporate Selection for a Subscriber Added By User " + strUser);
				...
				{
					String ret = rbtLogin.addCorporateSelection(strSub, song, bPrepaid, bReact, bRemSel, act_by, actInfo, corpSplFeatureBln,circleID);
					if (ret == null)
				...
				%>
				<jsp:forward page ="rbt_corp_tunes.jsp" />
			<%}
			else
			{
				...
				System.out.println("Corporate Selection for a File of Subs Added By User " + strUser);
					...
					while(line !=null)
					{
		               ...
		               Subscriber subscriber = Processor.getSubscriber(line);
						...
						String circleID = 	subscriber.getCircleID();
						if(subscriber.isValidPrefix() && subscriber.isCanAllow() && !subscriber.getStatus().equalsIgnoreCase(WebServiceConstants.SUSPENDED))
						   rbtLogin.addCorporateSelection(line, song, bPrepaid, bReact, bRemSel, act_by, actInfo, corpSplFeatureBln, circleID);
					   line = br.readLine();
					}
	...
	{
		System.out.println("Exception in rbt_corp_update "+e.getMessage());
