...
/**
 * RBT implementation class for the DNCTO {@link RuleEngine}.
 * 
 * @author vinayasimha.patil
 */
public class RBTRuleEngine implements RuleEngine
{
	private static Logger logger = Logger.getLogger(RBTRuleEngine.class);

	/*
	 * (non-Javadoc)
	 * @see com.onmobile.dnctoservice.plugin.RuleEngine#applyRules(java.io.File,
	 * java.io.File)
	 */
	/**
	 * Applies the RBT DNCTO rules and blacklisted numbers will be written to
	 * <tt>filteredFile</tt>. This method will be called by the DNCTO framework
	 * for filtering the blacklisted numbers for RBT promotions.
	 * 
	 * @param sourceFile
	 *            the reference to the file containing the complete DNCTO data
	 * @param filteredFile
	 *            the reference to the output filtered file
	 * @throws DNCTOException
	 *             if unable to read the DNCTO data file
	 */
	@Override
	public boolean applyRules(File sourceFile, File filteredFile)
			throws DNCTOException
	{
		if (sourceFile == null)
		{
		...
		Rule rule = RulesParser.parseRulesXML();
		...
		try
		{
			...
			while ((line = bufferedReader.readLine()) != null)
			{
				...
				JSONObject jsonObject = new JSONObject(line.substring(line
						.indexOf(DNCTOPluginUtil.delimiter) + 1));
