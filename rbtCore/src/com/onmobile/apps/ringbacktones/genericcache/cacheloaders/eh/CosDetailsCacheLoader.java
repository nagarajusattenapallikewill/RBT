 ...
 * EHCosDetailsBootstrapCacheLoaderFactory The class has a load method which
 * loads the parameters cache and is called by the EHCache when EHCache is
 * configured via a the RBT-EHCACHE.xml.
 * 
 * @author bikash.panda
 */
public class CosDetailsCacheLoader implements BootstrapCacheLoader
{
	/* (non-Javadoc)
	...
	/**
	 * This method loads the CosDetails cache. The data for the ehCache is returned
	...
	public void load(Ehcache ehCache) throws CacheException
	{
		...
		Map<String, List<CosDetails>> cacheMap = cacheGenerator.getCosDetailsMapForCaching();
		if (cacheMap != null)
		{
			Set<Entry<String, List<CosDetails>>> entrySet = cacheMap.entrySet();
			for (Entry<String, List<CosDetails>> entry : entrySet)
			{
				if (ehCache != null)
					ehCache.put(new Element(entry.getKey(), entry.getValue()));
			}
			Logger.getLogger(CosDetailsCacheLoader.class).info(" CosDetails Cache Loaded ");
		}
