 ...
 * EHChgargeClassBootstrapCacheLoaderFactory The class has a load method which
 * loads the parameters cache and is called by the EHCache when EHCache is
 * configured via a the RBT-EHCACHE.xml.
 * 
 * @author bikash.panda
 */
public class ChargeClassCacheLoader implements BootstrapCacheLoader
{
	/* (non-Javadoc)
	...
	public void load(Ehcache ehCache) throws CacheException
	{
		...
		Map<String, ChargeClass> cacheMap = cacheGenerator.getChargeClassMapForCaching();
		if (cacheMap != null)
		{
			Set<Entry<String, ChargeClass>> entrySet = cacheMap.entrySet();
			for (Entry<String, ChargeClass> entry : entrySet)
			{
				if (ehCache != null)
					ehCache.put(new Element(entry.getKey(), entry.getValue()));
			}
			Logger.getLogger(ChargeClassCacheLoader.class).info(" ChargeClass Cache Loaded ");
		}
