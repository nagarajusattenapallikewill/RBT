 ...
 * 
 */
public class CallGraphCreatorExecutor extends RbtThreadPoolExecutor
{
	private static Logger logger = Logger
			.getLogger(CallGraphCreatorExecutor.class);
	 ...
	 */
	private CallGraphCreatorExecutor(int corePoolSize, int maximumPoolSize,
			long keepAliveTime, TimeUnit unit, RbtBlockingQueue workQueue,
			RbtThreadFactory threadFactory, QueuePublisher queuePublisher)
	{
	...
	public static class Builder
	{
		...
		public Builder()
		{
			...
			threadFactory = new RbtThreadFactory("CallGraphCreatorExecutor");
			queuePublisher = new CallGraphCreatorPublisher(publishInterval,
					TimeUnit.SECONDS);
		}

		public CallGraphCreatorExecutor build()
		{
			if (logger.isInfoEnabled())
				logger.info("CallGraphCreatorExecutor Configuration: " + this);

			return new CallGraphCreatorExecutor(corePoolSize, maximumPoolSize,
