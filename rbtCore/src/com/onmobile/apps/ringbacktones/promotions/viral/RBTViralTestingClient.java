 ...
 *
 */
public class RBTViralTestingClient extends Thread {

	private static Logger logger = Logger.getLogger(RBTViralTestingClient.class);
	...
	public static List<String> configNumbersList = null;
	...
	private static RBTViralTestingClient instance = null;

	public static void main(String[] args){
		RBTViralTestingClient viral = RBTViralTestingClient.getInstance();
		...
		{
			viral.setName("VIRAL_TESTING_CLIENT");
			viral.start();
		}
	}

	private RBTViralTestingClient() {
	...
	public static RBTViralTestingClient getInstance() {
		if (instance == null) {
			synchronized (syncObject) {
				if (instance == null) {
					try {
						instance = new RBTViralTestingClient();
					}
					catch (Throwable e) {
						logger.error("", e);
						instance = null;
	...
	public void process() {
		...
		while(osc.hasNext()) {
			try 
			{
				...
				try
				{
					processOSCCdrFiles(oscCdrs, logFileExtractor);
					processNMSCdrFiles(nmsCdrs, logFileExtractor);
				}
				catch(Throwable e)
				{
					logger.error("", e);	
				...
				{
					OSCFileProcessor.processedCallerIds.clear();
	...
	private void processOSCCdrFiles(File[] cdrs, OSCnNMSLogExtracter logFileExtractor)
	{
		for (File file : cdrs) 
		{
			String absolutePath = file.getAbsolutePath();
			FileInfo fileInfo = FileInfoContainer.getFileInfo(absolutePath);
			
			if (fileInfo.hasToBeProcessed())
			{
				OSCFileProcessor processor = new OSCFileProcessor(fileInfo, logFileExtractor);
	...
	private void processNMSCdrFiles(File[] cdrs, OSCnNMSLogExtracter logFileExtractor)
	{
		for (File file : cdrs) 
		{
			String absolutePath = file.getAbsolutePath();
			...
			if (fileInfo.hasToBeProcessed())
			{
				NMSFileProcessor processor = new NMSFileProcessor(fileInfo, logFileExtractor);
				Thread t = new Thread(processor);
				t.start();
				allThreads.add(t);
			}
	...
	public void run(){
		...
		// loads all the object files into a hashmap
		FileInfoContainer.load();
		
		// gets all configured numbers into an arrayList
	...
	private void getConfiguredNumbers() 
	{
		if (configNumbersList == null)
		{
			String configNumbersStr = RBTViralConfigManager.getInstance().getParameter("CONFIGURED_NUMBERS");
			configNumbersList = new ArrayList<String>();

			if (configNumbersStr != null)
				configNumbersList = Arrays.asList(configNumbersStr.split(","));
		}
	}
...
//			InputSource is = new InputSource(new FileInputStream(xmlPath));
	...
	private File[] getLatestFiles(String cdr) {
		...
		logger.info("RBTViral::Files in CDR directory parsed " + cdrDir +" : " + length);
		return list;
	}
