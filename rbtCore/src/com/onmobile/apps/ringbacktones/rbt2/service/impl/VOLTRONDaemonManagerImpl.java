...
public class VOLTRONDaemonManagerImpl implements RBTSMDaemonService, iRBTConstant {

	private static Logger logger = Logger.getLogger(VOLTRONDaemonManagerImpl.class);
	...
	public VOLTRONDaemonManagerImpl() {
	...
	public HashMap<String, String> getSMURL(Subscriber subscriber, SubscriberStatus subscriberStatus,
		...
		String reqMode = null;
		if (subscriber != null) {
			operatorName = ServiceUtil.getOperatorName(subscriber).toUpperCase();
			...
			operatorName = ServiceUtil.getOperatorName(subscribers).toUpperCase(); //newly added
		...
		if (requestType.trim().equalsIgnoreCase("ACT") && isSelReactive) {
			smReqType += "_ACT";
			url = getURLStringWithOperatorName("REACTIVATION_URL", operatorName);
			...
			smReqType += "_ACT";
			url = getURLStringWithOperatorName("RETAILER_REACTIVATE_URL", operatorName);
			loggingInfoMap.put(SMHitLogger.TYPE, SMHitLogger.TYPE_REACTIVATION);
				...
				&& Arrays.asList(getParamAsString("MODES_FOR_ACTIVATION_URL_FOR_CONSENT").split(","))
						.contains(reqMode)) {
			smReqType += "_ACT";
			url = getURLStringWithOperatorName("MODE_WISE_ACTIVATION_URL", operatorName);
			if (!loggingInfoMap.containsKey(SMHitLogger.TYPE))
				loggingInfoMap.put(SMHitLogger.TYPE, SMHitLogger.TYPE_ACTIVATION);
		} else if (requestType.trim().equalsIgnoreCase("ACT")) {
			smReqType += "_ACT";
			url = getURLStringWithOperatorName("ACTIVATION_URL", operatorName);
			if (!loggingInfoMap.containsKey(SMHitLogger.TYPE))
				loggingInfoMap.put(SMHitLogger.TYPE, SMHitLogger.TYPE_ACTIVATION);
		} else if (requestType.trim().equalsIgnoreCase("DCT")) {
			smReqType += "_DCT";
			url = getURLStringWithOperatorName("DEACTIVATION_URL", operatorName);
			...
			if (subDownload != null && getParamAsString("BLOCK_RENEW_URL") != null)
				url = getURLStringWithOperatorName("BLOCK_RENEW_URL", operatorName);
		} else if (requestType.trim().equalsIgnoreCase("EVT")) {
			smReqType += "_EVT";
			url = getURLStringWithOperatorName("EVENT_URL", operatorName) + "srvkey=" + srvDefault + "&";
			srvKey = "eventkey";
			loggingInfoMap.put(SMHitLogger.TYPE, SMHitLogger.TYPE_EVENT);
		} else if (requestType.trim().equalsIgnoreCase("REN")) {
			smReqType += "_REN";
			url = getURLStringWithOperatorName("RENEWAL_URL", operatorName);
			isRenewal = true;
			loggingInfoMap.put(SMHitLogger.TYPE, SMHitLogger.TYPE_RENEWAL);
		} else if (requestType.trim().equalsIgnoreCase("REALTIME")) {
			smReqType += "_REAL";
			url = getURLStringWithOperatorName("REAL_TIME_CHARGING_URL", operatorName);
			String txnID = null;
			...
			smReqType += "_DIRDCT";
			url = getURLStringWithOperatorName("DIRECT_DEACT_URL", operatorName);
			msisdn = "cellno";
			subID = "91" + subID;
		...
		if (url != null) {
			...
			if (isChange) {
				...
				String change = getURLStringWithOperatorName("CHANGE_URL", operatorName);
				if (subscriber != null || subDownload != null || useUpgradeUrl)
					change = getURLStringWithOperatorName("UPGRADE_URL", operatorName);
	...
	private String getURLStringWithOperatorName(String param, String operatorName) {
		if (operatorName != null) {
			param = operatorName + "_" + param;
		}
