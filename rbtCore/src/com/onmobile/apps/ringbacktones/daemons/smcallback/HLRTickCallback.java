 ...
 */
public class HLRTickCallback implements SMCallback, SMCallbackConstants
{
	private static Logger logger = Logger.getLogger(HLRTickCallback.class);
			...
			SMCallbackContext smCallbackContext)
	{
		...
		SubscriberAnnouncements[] announcements = rbtDBManager
				.getActiveSubscriberAnnouncemetsForCallback(subscriberID);
				...
				&& !subscriber.subYes().equals(iRBTConstant.STATE_DEACTIVATED))
		{
					...
					&& extraInfoMap.get(iRBTConstant.EXTRA_INFO_PCA_FLAG).equalsIgnoreCase("TRUE"))
			{
				...
				{
					if (announcement.status() == iRBTConstant.ANNOUNCEMENT_ACTIVATION_PENDING)
					{
						announcement.setStatus(iRBTConstant.ANNOUNCEMENT_ACTIVE);
		...
		for (SubscriberAnnouncements announcement : announcements)
		{
			if (announcement.status() == iRBTConstant.ANNOUNCEMENT_ACTIVATION_PENDING)
			{
				announcement.setStatus(iRBTConstant.ANNOUNCEMENT_TO_BE_ACTIVED_PLAYER);
				isUpdated = rbtDBManager.updateAnnouncement(announcement);
			}
		}
			...
			SMCallbackContext smCallbackContext)
	{
		...
		for (SubscriberAnnouncements announcement : announcements)
		{
			if (announcement.status() == iRBTConstant.ANNOUNCEMENT_ACTIVATION_PENDING)
			{
				announcement.setDeactivationDate(new Date());
