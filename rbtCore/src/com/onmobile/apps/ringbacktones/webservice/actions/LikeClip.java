 ...
 * 
 */
public class LikeClip extends AbstractClipRatingAction
{
	private static Logger logger = Logger.getLogger(LikeClip.class);
	...
	public WebServiceResponse processAction(WebServiceContext webServiceContext)
	{
		...
		try
		{
			...
			ClipRating clipRating = RBTCacheManager.getInstance().likeClip(
					clipID);
			if (clipRating != null)
				response = SUCCESS;
			else
