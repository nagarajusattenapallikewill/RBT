...
import com.onmobile.amoeba.common.util.RBTUtils;
...
public class RBTContent implements WebServiceConstants
{
	...
	private String browsingLanguage = null;
	...
	private RBTContent()
	{
		...
		try
		{
			...
			String httpSoTimeOutStr = getRBTServiceParameter("HTTP_SOCKET_TIMEOUT"); 
			...
			String httpMaxHostConnectionsStr = getRBTServiceParameter("MAX_HOST_HTTP_CONNECTIONS"); 
			if (httpMaxHostConnectionsStr != null)
			...
			browsingLanguage = getRBTServiceParameter("DEFAULT_BROWSING_LANGUAGE");
	...
	public static RBTContent getInstance()
	{
		if (rbtContent == null)
		{
			synchronized (RBTContent.class)
			{
				if (rbtContent == null)
			...
			String circleID, String isPrepaid, String language, int pageNo)
	{
		try
		{
			...
			if (browsingLanguage != null)
				requestParams.put(param_browsingLanguage, browsingLanguage);
	...
	private void addToContentMap(String contentType, int pageNo, String key, String content)
	{
		if (content == null)
			return;
	...
	public void clearContentMap(int contentID)
	{
		...
		RBTUtils.clearRBTContentCacheForKeyStartingWith(contentID); 
	}
	...
	synchronized public void clearContentMap(String key)
	{
		...
		try
		{
			RBTUtils.clearRBTContentCacheForKey(key);
		}
		catch (Exception e)
		{
			RBTLogger.logException(CLASSNAME, "clearContentMap", e);
