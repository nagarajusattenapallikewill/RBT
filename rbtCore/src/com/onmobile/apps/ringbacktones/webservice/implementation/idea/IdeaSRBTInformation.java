...
import com.onmobile.apps.ringbacktones.webservice.implementation.BasicSRBTXMLElementGenerator;

public class IdeaSRBTInformation extends IdeaRBTInformation{

	private static Logger logger = Logger.getLogger(IdeaSRBTInformation.class);
	
	/**
	 * @throws ParserConfigurationException
	 */
	public IdeaSRBTInformation() throws ParserConfigurationException
	{
		super();
	}
	...
	public Document getSpecificRBTInformationDocument(WebServiceContext task)
	{
		...
		logger.info("getSpecificRBTInformationDocument: " + task.getString(param_info));
		addCharegeClassElementInLibrary(document, task);
		return document;
	}
	...
	{
		logger.info("getSubscriberElement invoked");
		...
		logger.info("getSubscriberElement invoked done");
		return element;
	}
	...
	protected void addCharegeClassElementInLibrary(Document document, WebServiceContext task){
		...
		if (libraryElem != null)
		{
			...
			if (nextChargeClassAttr == null)
			{
				...
				logger.info("NextChargeClass: " + nextChargeClass);
				if (nextChargeClass != null)
				{					
					ChargeClass chargeClass = CacheManagerUtil.getChargeClassCacheManager().getChargeClass(nextChargeClass);
					logger.info("NextChargeClass: " + chargeClass.getChargeClass());
				...
				logger.info("adding next charge class");
			}
		}
		logger.info("invoked");
	}
