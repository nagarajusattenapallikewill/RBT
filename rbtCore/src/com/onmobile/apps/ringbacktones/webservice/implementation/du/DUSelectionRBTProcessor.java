...
public class DUSelectionRBTProcessor extends BasicRBTProcessor {

	private static Logger logger = Logger
			.getLogger(DUSelectionRBTProcessor.class);
	...
	public String processSelection(WebServiceContext task) {
		...
		try {
			...
			if (task.containsKey(param_chargeClass)
					&& overrideChargeClassMap.containsKey(task
							.getString(param_chargeClass))) {
				task.put(param_chargeClass, overrideChargeClassMap.get(task
						.getString(param_chargeClass)));
			}
			...
			if (action.equalsIgnoreCase(action_overwrite)
				...
				SubscriberStatus subscriberStatus = rbtDBManager
						.getActiveSubscriberRecord(subscriberID, callerId,
			...
			if (!isContentAllowed) {
				...
				if (subscriber != null) {
					Map<String, String> upgradeCodIdMap = MapUtils
							.convertToMap(upgrdCosID, ";", "=", ",");
					if (upgradeCodIdMap != null
							&& upgradeCodIdMap.containsKey(subscriber.cosID())) {
						String upgradeCosID = upgradeCodIdMap.get(subscriber
								.cosID());
			...
			if (category != null
				...
				if (!rbtDBManager.isSubscriberDeactivated(subscriber)) {
					...
					if (extraInfoMap != null
						...
						String[] packs = (packStr != null && packStr.trim()
								.length() > 0) ? packStr.trim().split(",")
								: null;
			...
			if (task.containsKey(param_cosID)) {
				String songBasedCosId = CacheManagerUtil
						.getParametersCacheManager().getParameterValue(
								iRBTConstant.COMMON, "SONG_BASED_COS_ID", null);
				if (songBasedCosId != null) {
					List<String> cosIdsList = Arrays.asList(songBasedCosId
							.split(","));
					String cosId = task.getString(param_cosID);
					Subscriber subscriber = DataUtils.getSubscriber(task);
					if (cosIdsList.contains(cosId)
						...
						if (task.containsKey(param_profileHours)
								|| task.containsKey(param_cricketPack)
								|| (task.getString(param_status) != null && task
										.getString(param_status)
										.equalsIgnoreCase("99"))
								|| (task.containsKey(param_callerID) && !task
			...
			boolean isSubscriberAlreadyNotDeactive = rbtDBManager
					.isSubscriberActivated(subscriberID);
			logger.info("isSubscriberAlreadyNotDeactive = "
					+ isSubscriberAlreadyNotDeactive);
			if (Arrays.asList(
					getParamAsString("COMMON",
							"MODES_FOR_CHECKING_IF_COMBO_REQUEST_PENDING", "")
							.split(",")).contains(task.getString(param_mode))) {
				List<DoubleConfirmationRequestBean> baseConsentPendingRecordList = rbtDBManager
						.getConsentPendingRecordListByMsisdnNType(subscriberID,
								"ACT");
				List<DoubleConfirmationRequestBean> selConsentPendingRecordList = rbtDBManager
						.getConsentPendingRecordListByMsisdnNType(subscriberID,
								"SEL");
				if (baseConsentPendingRecordList != null
						&& selConsentPendingRecordList != null
			...
			if (!(task.containsKey(param_isPreConsentBaseSelRequest) && response
					.equalsIgnoreCase(preConsentBaseSelSuccess))) {
				if (!response.equalsIgnoreCase(SUCCESS)
			...
			if (subscriber == null
					&& task.containsKey(param_isPreConsentBaseSelRequest)) {
				isPrepaid = task.get("IS_PREPAID_CONSENT") != null ? (Boolean) task
						.get("IS_PREPAID_CONSENT") : false;
			...
			// For Idea Consent. If active user makes profile selection, then
			// mode will be passed as configured mode
			String profileMode = RBTParametersUtils.getParamAsString("COMMON",
					"PROFILE_MODE_FOR_ACTIVE_USER", null);
			boolean isSubActivated = iRBTConstant.STATE_ACTIVATED
					.equals(subYes);
			...
			/*
			 * If below parameter is TRUE, we allow overlapping Time of Day and
			 * Day of Week selections.
			 * 
			 * If we set doTODCheck to false, then at DB layer, we don't check
			 * for the overlapping selections.
			 */
			boolean isOverlapAllowed = RBTParametersUtils.getParamAsBoolean(
					"COMMON", "IS_OVERLAP_ALLOWED_FOR_TIME_AND_DAY_SELECTIONS",
					"FALSE");
			...
			if (task.getString(param_action) != null
					&& task.getString(param_action).equalsIgnoreCase(
							action_acceptGift)
					&& !task.containsKey(param_offerID)) {

				boolean allowBaseOffer = RBTParametersUtils.getParamAsBoolean(
						"GIFT", iRBTConstant.ALLOW_GET_OFFER, "FALSE");
				if (allowBaseOffer) {
					com.onmobile.apps.ringbacktones.smClient.beans.Offer[] offer = RBTSMClientHandler
							.getInstance().getOffer(subscriberID,
									...
									Offer.OFFER_TYPE_SELECTION, null, null,
									null);
			...
			if (RBTParametersUtils.getParamAsBoolean("COMMON",
					iRBTConstant.ALLOW_BASE_OFFER_DU, "FALSE")
				...
				if (RBTParametersUtils.getParamAsBoolean("COMMON",
					...
					rbtDetailsRequest
							.setOfferType(Offer.OFFER_TYPE_SELECTION_STR);
				...
				String offerDaysLimit = RBTParametersUtils.getParamAsString(
						"COMMON", iRBTConstant.OFFER_DAYS_LIMIT, "");
				Date date = new Date();
				logger.info("Going for Selection Offer....");
				logger.info("SubscriberID = " + subscriberID + " , Mode = "
						+ task.getString(param_mode)
						+ " , Offer Type = 2(Selection) , ClassType = "
						+ classType);
				// Selection Model Changes
				SubscriberStatus[] activeSubscriberSelections = rbtDBManager
						.getActiveSubscriberStatus(subscriberID, callerId,
								fromTime, toTime);
				boolean isActPendingRecord = false;
				if (activeSubscriberSelections != null && offerId != null
						&& !offerId.equalsIgnoreCase("-1")) {
					for (SubscriberStatus subSelection : activeSubscriberSelections) {
						String xtraInfo = subSelection.extraInfo();
						HashMap<String, String> xtraInfoMap = DBUtility
								.getAttributeMapFromXML(xtraInfo);
						String selStatus = subSelection.selStatus();
						if ((selStatus
								.equalsIgnoreCase(iRBTConstant.STATE_TO_BE_ACTIVATED) || selStatus
								.equalsIgnoreCase(iRBTConstant.STATE_ACTIVATION_PENDING))
								&& xtraInfoMap != null
				...
				if (isActPendingRecord) {
					logger.info("Already one selection is pending to be charged.So, Not allowing to selection the song");
					return NOT_ALLOWED;
				}

				String offerIdForBlacklisted = RBTParametersUtils
						.getParamAsString("COMMON",
				...
				if (Arrays.asList(offerIdForBlacklisted.split(",")).contains(
					...
					String smsText = CacheManagerUtil.getSmsTextCacheManager()
							.getSmsText("OFFER", "BLACKLISTED_OFFER_SEL_TEXT",
									language);
			...
			for (String callerID : validCallerIds) {
				if (category.getCategoryTpe() == iRBTConstant.DYNAMIC_SHUFFLE) {
					...
					isNoConsentRequired = isNoConsentRequired(task, subscriber,
							isSubscriberAlreadyNotDeactive, classType, mode);
					// logger.info("isSubscriberAlreadyNotDeactive:"+isSubscriberAlreadyNotDeactive+
					// " classType :" + classType + " mode:"+mode +
					// "isNoConsentRequired :" + isNoConsentRequired);
					...
					for (int i = 0; i < clips.length; i++) {
						...
						if (isSupportSMClientModel(task, SELECTION_OFFERTYPE)) {
							...
							if (task.containsKey(param_refID)
									&& !Utility
											.isModeConfiguredForIdeaConsent(selectedBy)) {
							...
							if (task.containsKey("CONSENT_SUBSCRIPTION_INSERT")) {
								clipMap.put("CONSENT_SUBSCRIPTION_INSERT",
										"true");
							}
							...
							if (task.containsKey(param_isUdsOn)) {
								clipMap.put(param_isUdsOn,
										task.getString(param_isUdsOn));
							}
						...
						if (clipMap.containsKey("RECENT_CLASS_TYPE")) {
							task.put("RECENT_CLASS_TYPE",
									clipMap.remove("RECENT_CLASS_TYPE"));
						}
						...
						if (response.equalsIgnoreCase(SUCCESS)) {
							...
							task.put("CONSENTCLASSTYPE",
									clipMap.remove("CONSENTCLASSTYPE"));
							task.put("CONSENTSUBCLASS",
									clipMap.remove("CONSENTSUBCLASS"));
							task.put("CONSENT_SERVICE_ID",
									clipMap.remove("CONSENT_SERVICE_ID"));
							task.put("CONSENT_SERVICE_CLASS",
									clipMap.remove("CONSENT_SERVICE_CLASS"));
					...
					String contentBaseClasstype = getContentBasedClassType(
							task, clip);
					...
					// RBT-10785
					boolean addProtocolNumber = RBTParametersUtils
							.getParamAsBoolean("WEBSERVICE",
									"ADD_PROTOCOL_NUMBER", "FALSE");
					if (addProtocolNumber) {
						String wavFile = null;
						if (clip != null) {
							wavFile = clip.getClipRbtWavFile();
						}// Selection Model Changes
						SubscriberStatus subscriberSelection = rbtDBManager
								.getActiveSubscriberRecord(subscriberID,
										callerId, status, fromTime, toTime);
						if (null == subscriberSelection
								|| (null != subscriberSelection && subscriberSelection
										.selStatus().equalsIgnoreCase(
												iRBTConstant.STATE_DEACTIVATED))) {
							selectionInfo = appendProtocolNumber(subscriberID,
									selectionInfo);
						} else {
							logger.warn("Since selection already exists, not appending protocol number. subscriberID: "
					...
					if (task.containsKey(param_isPreConsentBaseSelRequest)
						...
						selResponse = rbtDBManager.addSelectionConsent(
								...
								extraInfoMap, requestType, 0, subscriber, task,
								categoriesObj, doTODCheck);
					...
					if (clipMap
							.containsKey(iRBTConstant.param_isSelConsentInserted)) {
						task.put(
								iRBTConstant.param_isSelConsentInserted,
	...
	public String processActivation(WebServiceContext task) {
		...
		try {
			...
			if (task.containsKey(param_subscriptionClass)
					&& overrideSubscriptionClassMap.containsKey(task
							.getString(param_subscriptionClass))) {
				task.put(param_subscriptionClass, overrideSubscriptionClassMap
						.get(task.getString(param_subscriptionClass)));
			}
			...
			if (!isUnsubDelayedTimeConfiguredForSms && subXtraInfoMap != null
					&& subXtraInfoMap.containsKey("UNSUB_DELAY")
					...
					if (isSupportSMClientModel(task, BASE_OFFERTYPE)) {
						...
						if (cosID != null) {
							...
							if (task.containsKey(param_cosID)) {
								...
								if (songBasedCosId != null) {
									List<String> cosIdsList = Arrays
											.asList(songBasedCosId.split(","));
									String cosId = task.getString(param_cosID);
									if (cosIdsList.contains(cosId)) {
										SubscriberStatus[] activeSubscriberStatus = rbtDBManager
												.getAllActiveSubscriberSettings(subscriber
														.subID());
									...
									if (cosIdsList.contains(cosID)
											&& !cosIdsList.contains(cosId)) {
										extraInfoMap
												.put(iRBTConstant.UDS_OPTIN,
														"FALSE");
									}
								}
							}
						...
						if (subscriber.subYes().equals(
							...
							if (userInfoMap
								...
								extraInfoMap
										.put(iRBTConstant.EXTRA_INFO_TPCGID,
												userInfoMap
														.get(iRBTConstant.EXTRA_INFO_TPCGID));
							}
							// RBT-9213
							if (userInfoMap
									.containsKey(Constants.param_sdpomtxnid)) {
								extraInfoMap
										.put(Constants.param_sdpomtxnid,
												userInfoMap
														.get(Constants.param_sdpomtxnid));
							}
							if (userInfoMap
									.containsKey(Constants.param_seapitype)) {
								extraInfoMap
										.put(Constants.param_seapitype,
												userInfoMap
														.get(Constants.param_seapitype));
							}
							// Added extra info column to update the sr_id and
							// originator info
							...
							if (userInfoMap
									.containsKey(Constants.param_ORIGINATOR)) {
								extraInfoMap
										.put(Constants.param_ORIGINATOR,
												userInfoMap
														.get(Constants.param_ORIGINATOR));
							}
							if (userInfoMap.containsKey(Constants.param_vendor
									.toUpperCase())) {
								extraInfoMap.put(Constants.param_vendor
										.toUpperCase(), userInfoMap
										.get(Constants.param_vendor
												.toUpperCase()));
							}
							// end
							...
							subExtraInfo.put(
									iRBTConstant.EXTRA_INFO_OLD_ACT_BY,
									oldActBy);
							extraInfo = DBUtility
							...
							if (task.containsKey(WebServiceConstants.param_isPreConsentBaseRequest)) {
								...
								logger.info("Updated status changed, update "
									...
									.containsKey(WebServiceConstants.param_isPreConsentBaseSelRequest)) {
								success = rbtDBManager
										.convertSubscriptionType(subscriberID,
												subscriber.subscriptionClass(),
												subscriptionClass, activatedBy,
												activationInfo,
												concatActivationInfo,
												newRbtType, true, extraInfo,
												subscriber);
								logger.info("Updated status changed, update "
										+ "status: " + success
										+ " for subscriber: " + subscriberID);
			...
			CosDetails cos = null;
			boolean isUpgradeDownloadLimitSongPack = RBTParametersUtils
					.getParamAsBoolean("COMMON",
							"UPGRAGE_DOWNLOAD_LIMIT_SONG_PACK", "FALSE");
			if (task.containsKey(param_cosID)) {
			...
			// if(cos == null && task.containsKey(param_cosID) &&
			// RBTParametersUtils.getParamAsBoolean("COMMON","UPGRAGE_DOWNLOAD_LIMIT_SONG_PACK",
			// "FALSE")) {
			...
			// cos =
			// CacheManagerUtil.getCosDetailsCacheManager().getCosDetail(task.getString(param_cosID));
			// }
			// }
			// }
			...
			if (!rbtDBManager.isSubscriberDeactivated(subscriber)) {
				...
				if (task.containsKey(param_packCosId)) {
					...
					if (cosDetails != null
						...
						if (rbtDBManager
							...
							String packDeactPending = isSongPackDeactivationPending(
									subscriberID, subExtraInfoMap);
							...
							if (isDownloadSongPackOverLimitReached(cosID,
									subscriber.subID(), task)) {
								return OVERLIMIT;
							}

							if (task.containsKey("PACK_NUM_MAX_SELECTON")) {
								packNumMaxSelection = Integer
										.parseInt((String) task
												.remove("PACK_NUM_MAX_SELECTON"));
							}
							...
							if (packNumMaxSelection != -1) {
								provisioningReqs
										.setNumMaxSelections(packNumMaxSelection);
							}
							if (rbtDBManager
				...
				if (!isDirectActivation) {
					logger.warn("Not processing activation, returning "
							+ status + ", request is not direct activation");
			...
			if ((task.containsKey(param_isPreConsentBaseRequest) || task
					.containsKey(param_isPreConsentBaseSelRequest))
					&& Arrays
							.asList(iRBTConstant.COMMON, "ADRBT_SUB_CLASS", "")
			...
			if (Arrays.asList(
				...
				userInfoMap.put(iRBTConstant.EXTRA_INFO_PRE_RBT_WAV,
						preRbtWavFile);
			}
			...
			if (task.getString(param_action) != null
					&& task.getString(param_action).equalsIgnoreCase(
							action_acceptGift)
					&& ((!task.containsKey(param_requestFromSelection) && !task
							.containsKey(param_offerID)) || (task
							.containsKey(param_requestFromSelection) && !task
							.containsKey(param_subscriptionOfferID)))) {

				boolean allowBaseOffer = RBTParametersUtils.getParamAsBoolean(
						"GIFT", iRBTConstant.ALLOW_GET_OFFER, "FALSE")
						|| RBTParametersUtils.getParamAsBoolean("GIFT",
								iRBTConstant.ALLOW_ONLY_BASE_OFFER, "FALSE");
				if (allowBaseOffer) {
					com.onmobile.apps.ringbacktones.smClient.beans.Offer[] offer = RBTSMClientHandler
							.getInstance().getOffer(subscriberID, activatedBy,
									Offer.OFFER_TYPE_SUBSCRIPTION, null,
									cos.getSmsKeyword(), null);
					if (offer != null && offer.length > 0) {
						subscriptionClass = offer[0].getSrvKey();

						if (task.containsKey(param_requestFromSelection)) {
							task.put(param_subscriptionOfferID,
									offer[0].getOfferID());
						} else {
							task.put(param_offerID, offer[0].getOfferID());
						}
			...
			if (RBTParametersUtils.getParamAsBoolean("COMMON",
					iRBTConstant.ALLOW_BASE_OFFER_DU, "FALSE")
					|| RBTParametersUtils.getParamAsBoolean("COMMON",
							iRBTConstant.ALLOW_BASE_OFFER, "FALSE")) {
				...
				if (RBTParametersUtils.getParamAsBoolean("COMMON",
					...
					// com.onmobile.apps.ringbacktones.smClient.beans.Offer[]
					// offer =
					// RBTSMClientHandler.getInstance().getOffer(subscriberID,
					// activatedBy,
					// Offer.OFFER_TYPE_SUBSCRIPTION, null, cos.getSmsKeyword(),
					// null);
					if (offers != null && offers.length > 0) {
				...
				logger.info("SubscriberID = "
						...
						+ " , OFFER_ID = "
						+ offerId
						+ " , Offer Type = 2(Subscription) , SubscriptionClass = "
						+ subscriptionClass);
				...
				if (Arrays.asList(offerIdForBlacklisted.split(",")).contains(
					...
					task.put(param_receiverID, subscriberID);
					String smsText = CacheManagerUtil.getSmsTextCacheManager()
							.getSmsText("OFFER", "BLACKLISTED_OFFER_BASE_TEXT",
									null);
					task.put(param_smsText, smsText);
					sendSMS(task);
					return SUCCESS;
			...
			if (isSupportSMClientModel(task, offerType)) {
				...
				// RBT-10785
				boolean addProtocolNumber = RBTParametersUtils
						.getParamAsBoolean("WEBSERVICE", "ADD_PROTOCOL_NUMBER",
								"FALSE");
				if (addProtocolNumber) {
					activationInfo = appendProtocolNumber(subscriberID,
							activationInfo);
				}
				if (task.containsKey(param_isUdsOn)) {
					xtraParametersMap.put(param_isUdsOn,
							task.getString(param_isUdsOn));
				}
				...
				if (xtraParametersMap
						.containsKey("CONSENT_SUBSCRIPTION_INSERT")) {
					response = SUCCESS;
				...
				boolean is121TnbEnabled = Boolean
						.parseBoolean(RBTParametersUtils.getParamAsString(
								"COMMON", "121_TNB_SUBSCRIPTION_CLASS_ENABLED",
								"FALSE"));
			...
			if (!task.containsKey(param_isPreConsentBaseRequest)
					&& !task.containsKey(param_isPreConsentBaseSelRequest)
					&& !task.containsKey("CONSENT_SUBSCRIPTION_INSERT")) {
				...
				if (!rbtDBManager.isSubscriberDeactivated(subscriber)) {
					...
					// Tef-Spain yavoy new RBT service re-pricing RBT-11113 : to
					// remove expired donwloads with status
					// STATE_DOWNLOAD_SEL_TRACK(t).
					rbtDBManager.removeDeactivateSubscriberDownloads(subscriber
							.subID());
