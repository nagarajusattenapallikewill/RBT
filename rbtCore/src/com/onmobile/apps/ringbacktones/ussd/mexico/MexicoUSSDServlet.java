package com.onmobile.apps.ringbacktones.ussd.mexico;
...
public class MexicoUSSDServlet extends HttpServlet {

	private static final long serialVersionUID = 6414086072576094870L;

	private static Logger basicLogger = Logger.getLogger(MexicoUSSDServlet.class);
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		response.setContentType(MexicoUSSDServlet.CONTENT_TYPE_PLAIN_REQUEST);
		...
		if(invalidResp!=null&&invalidResp.equalsIgnoreCase("invalidresponse")){
			String resp=MexicoUSSDResponseBuilder.getNormalResponse(USSDConfigParameters.getInstance().getParameter("ERROR_MESSAGE"),StringUtils.processInputParameters(input), (String)input.get("next"), StringUtils.processInputParameters(input));
			response.setContentType(CONTENT_TYPE_REQUEST_ANSWER);
			response.getWriter().println(resp);
		}
		else if("mainmenu".equals(action) || StringUtils.isEmpty(action)) {

			//MexicoUSSDMainMenu mainMenu = new MexicoUSSDMainMenu(input, response);
			//mainMenu.process();
			heading=USSDConfigParameters.getInstance().getParameter("TOPMESSAGE_MAIN_MENU");
			message=USSDConfigParameters.getInstance().getParameter("MAIN_MENU_ITEMS");
			new MexicoUSSDMessageDisplay(input,response,heading,message,action,"mainmenu",null).displayMessage();
		}else{
			//default
