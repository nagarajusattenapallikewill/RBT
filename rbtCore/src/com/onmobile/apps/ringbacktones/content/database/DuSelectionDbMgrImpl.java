...
public class DuSelectionDbMgrImpl extends RBTDBManager {

	private static Logger logger = Logger.getLogger(DuSelectionDbMgrImpl.class);
	...
	public boolean checkUserStatuskForConsentFlow(Subscriber sub) {
		...
		return (null == sub || sub.subYes().equalsIgnoreCase("X")) ? true
				: false;
		...
		try {
			...
			if (sub.rbtType() == TYPE_RBT_RRBT
				...
				if (delSelections) {
					...
					boolean isDeactSelections = SubscriberStatusImpl
							.deactivate(conn, subID(subscriberID), date,
									smDeactivation, false, deactivate, rbtType);

					logger.info("Deactivated subscriber "
							+ "selections. " + ", isDeactSelections: "
	...
	public Subscriber activateSubscriber(String subscriberID, String activate,
		...
		try {
			...
			if (subscriber != null) {
				...
				if (subscriber != null) {
					logger.info("Successfully created an entry in the consent table for subscriber ID : "
							+ subscriberID);
					return subscriber;
				}
			...
			subscriber = SubscriberImpl.insert(conn, subID(subscriberID),
					activate, null, startDate, endDate, prepaid, null, null, 0,
					activationInfo, subscriptionClass, null, null, null,
					subscription, 0, cosID, cosID, rbtType, null,
	...
	public String canBeGifted(String subscriberId, String callerId,
		...
		if (contentID != null && !contentID.equals("null")
			...
			SubscriberStatus[] settings = getAllActiveSubSelectionRecords(callerId);
			if (settings != null) {
				for (SubscriberStatus subscriberStatus : settings) {
					if ((isClip && subscriberStatus.subscriberFile().equals(
							clipName))
							|| subscriberStatus.categoryID() == catID) {
