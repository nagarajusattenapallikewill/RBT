 ...
 * $Id: ViralSMSNewImpl.java,v 1.10 2015/04/20 06:16:54 balachandar.p Exp $
 * $Revision: 1.10 $
...
public class ViralSMSNewImpl extends RBTPrimitive implements ViralSMSTable
{
	private static Logger logger = Logger.getLogger(ViralSMSNewImpl.class);

    public static final String TABLE_NAME = "RBT_VIRAL_NEW";
	...
	public ViralSMSNewImpl(String subscriberID, Date smsTime, String type,
	...
	static boolean removeViralSMSesByType(Connection conn, String callID, String[] smsTypes) {
		Statement stmt = null;
		ResultSet results = null;
		...
		String query = "DELETE FROM " + TABLE_NAME + " WHERE " + CALLER_ID_COL + " = " + sqlString(callID) + " AND " + SMS_TYPE_COL + " IN ( " + types + "  )";
		...
		try
		{
			...
			if(count > 0) {
				logger.info("Successfully records removed from viralsmstable");
				return true;
			}
			else {
				logger.info("Records not removed from viralsmstable");
				return false;
			}
		}
		catch(SQLException se)
	...
	static boolean updateLatestViralSMSTypeOfCaller(Connection conn, String subscriberID, String oldType, String newType, int time , HashMap<String,String> extraInfoMap)
	{
		...
		{
			stmt = conn.createStatement();
			ResultSet results = stmt.executeQuery(tempQuery);
			if (results.next())
			{
				sentTime = results.getTimestamp(SMS_SENT_TIME_COL);
				extraInfo = results.getString(EXTRA_INFO_COL);
