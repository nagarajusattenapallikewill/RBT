...
import com.onmobile.apps.ringbacktones.content.TrialSelection;

public class TrialSelectionImpl extends RBTPrimitive implements TrialSelection
{
	private static Logger logger = Logger.getLogger(TrialSelectionImpl.class);
	
	static String tableName = "RBT_TRIAL_SELECTION";
	...
	private static String seqName = "RBT_TRIAL_SEQUENCE";
	...
	private TrialSelectionImpl(long seqId, String subId, String circleId,
	...
	static TrialSelection[] getViralSMS(Connection conn, long a_seqId, String a_circleId, String a_chargePack, 
			Date a_startDate, int a_iterId, int a_count)
	{
		...
		TrialSelection trialSelection = null;
		ArrayList<TrialSelection> trialSeelctionList = new ArrayList<TrialSelection>();
		...
		try
		{
			...
			while (results.next())
			{
				...
				trialSelection = new TrialSelectionImpl(seqId, subscriberID, a_circleId, chargePack, startDate, iterId);
				trialSeelctionList.add(trialSelection);
			}
		}
		catch(SQLException se)
		{
		...
		if(trialSeelctionList.size() > 0)
		{
			logger.info("RBT::retrieving records from RBT_TNB_SUBSCRIBER successful");
			return (TrialSelection[])trialSeelctionList.toArray(new TrialSelection[0]);
		} 
		else
		{
	...
	public static TrialSelectionImpl getNextTnbSubscriber(ResultSet results) throws SQLException
	{
		...
		TrialSelectionImpl trialSelectionImpl = new TrialSelectionImpl(seqId, subscriberID, circleId, chargePack, startDate, iterId);
		return trialSelectionImpl;
	}
