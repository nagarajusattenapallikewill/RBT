...
public class VirginDbMgrImpl extends RBTDBManager{
			...
			String selInterval, HashMap extraInfo, boolean useUIChargeClass, String refID, boolean isDirectActivation)
	{
		...
		try
		{
			 ...
			 */
			String udpId = null;
			if(extraInfo.containsKey("UDP_ID"))
			...
			if (subscriberStatus == null)
			{
				...
				if (subscriberStatus != null)
				{
					if(inLoop && subscriberStatus.categoryType() == SHUFFLE)
						inLoop = false;
					//RBT-12830
					else if(inLoop && subscriberStatus.status() == 1 && subscriberStatus.selType() != 2 && rbtType == 2)
						inLoop = false;
				}
...
//					oldSubClass = sub.oldClassType();
				...
				boolean isFirstProfileSong =  isFirstProfileSong(m_overridableSelectionStatus,status,selCount) ;
				boolean isSpecialCallerSetting = isSpecialCallerSelection(callerID);
				if (m_overridableCategoryTypes.contains(""+categories.type()) || isFirstProfileSong)
					incrSelCount = false;
					...
					if (subPacks != null)
					{
						...
						{//Idea-Combo DT New Model RBT-14087
							nextClass = getNextChargeClass(sub,
									specialSelection, inLoop, selectionCount, clipClassType, isSpecialCallerSetting);
						}
					}
					else
					{//Idea-Combo DT New Model RBT-14087
						nextClass = getNextChargeClass(sub, specialSelection,
						...
						for(int i=0;subscriberSelections != null && i<subscriberSelections.length;i++)
						{
							if(subscriberSelections[i].selType() == 2)
							{
								...
								if(campaignId != -1)
								{
									...
									if(m_corporateDiscountChargeClass != null && null != bulkUploadTask && bulkUploadTask.getTaskMode() != null &&
											m_corporateDiscountChargeClass.containsKey(bulkUploadTask.getTaskMode()))
									{
										...
										if(discountClassMap != null && classType != null && discountClassMap.containsKey(classType))
	...
	/*
	 * RBT-7621::Idea Consent URL
	 * It inserts the Activation request into RBT_CONSENT table
	 * instead of RBT_SUBSCRIBER table.
	 */
	 public String insertPreConsentSubscriptionRequest(String subscriberID, String activate,
			Date startDate, Date endDate, boolean isPrepaid, int activationTimePeriod,
			int freePeriod, String actInfo, String classType,
			boolean smActivation, CosDetails cos, boolean isDirectActivation, int rbtType, HashMap extraInfo, String circleId, String refId)
     {
		Connection conn = getConnection();
		if(conn == null)
			return null;
		...
		try
		{
			...
			int secondsToBeAddedInRequestTime = DBUtility.secondsToBeAddedInRequestTime(circleId, activate);
			if (secondsToBeAddedInRequestTime != -1) {
				Calendar cal = Calendar.getInstance();
				cal.add(Calendar.SECOND, secondsToBeAddedInRequestTime);
				requestTime = cal.getTime();
			}
			success = ConsentTableImpl.insertSubscriptionRecord(conn, subID(subscriberID), activate,
					startDate, endDate, actInfo, prepaid,
					subscriptionClass,cosID,
					rbtType, isDirectActivation, subExtraInfo, circleId,refId,0,false, requestTime,null);
			if(success)
				response = "success";
		}
		catch(Throwable e)
...
/*			if(cos != null && !cos.isDefault())
			...
			//For Idea 121 consent
			if(refId != null && !com.onmobile.apps.ringbacktones.webservice.common.Utility.isModeConfiguredForIdeaConsent(activate)) {
				finalRefID = refId;
			}
			
			if (subscriber != null && (!Utility.isModeConfiguredForConsent(activate) || extraInfo.containsKey(iRBTConstant.EXTRA_INFO_TPCGID))) {
				...
				if(!extraInfo.containsKey(iRBTConstant.EXTRA_INFO_TPCGID)) {
					try 
					{
						//RBT-9873 Added xtraParametersMap for CG flow
