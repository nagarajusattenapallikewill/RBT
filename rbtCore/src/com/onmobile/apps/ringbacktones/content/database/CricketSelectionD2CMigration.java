...
public class CricketSelectionD2CMigration extends RBTDBManager implements ID2CMigration {
	...
	public SubscriberStatus[] getSelection(String subscriberID) {
		...
		if (conn == null)
			return null;
		try {
			SubscriberStatus[] cricketSel = SubscriberStatusImpl.getActiveSelectionsByStatus(conn, subscriberID, 90);
			List<SubscriberStatus> activeStatus = new ArrayList<SubscriberStatus>();
			if(cricketSel == null || cricketSel.length < 1){
				return null;
			}
			
			for (SubscriberStatus subscriberStatus : cricketSel) {
